...
export interface MenuVersionContent {
  ...
  /**
   * §39 · la nota de alérgenos del menú entero (RN-ALE-01). Opcional
   * porque una versión guardada antes de la migración 102 no la tiene, y
   * "no la tiene" no es lo mismo que "está vacía".
   */
...
export interface MenuDiff {
  ...
  /** RN-ALE-06 · la nota de alérgenos, si cambió. */
  readonly allergenNote: FieldDiff<string | null> | null;
...
export function diffMenuVersions(
  ...
  const allergenNote = mismoTexto(antes.allergenNote ?? null, despues.allergenNote ?? null)
    ? null
    : { before: antes.allergenNote ?? null, after: despues.allergenNote ?? null };
  ...
  return { courses, drink, price, note, allergenNote, identical };
}
