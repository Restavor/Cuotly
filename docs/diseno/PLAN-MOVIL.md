...
| P5 | Los **filtros van en una fila** de dos o tres desplegables | Uno debajo de otro a lo ancho, con "Filtrar" | `components/ui/FilterBar.tsx` · **hecho (24/09)**; dos por fila y no tres desde la parte 10, para que no se corte el valor |
...
| 24/09/2026 | **Parte 9 · Equipo, Planes, Agente y Ajustes (páginas 88 a 109)** · barridas a 390 px las cuatro pestañas de Equipo, las cuatro de Planes con el detalle de un restaurante, Agente y las ocho vistas de Ajustes con Suscripción, Auditoría, Exportación y Propiedad: ninguna se sale. **Rejillas que no se apilan**: la disponibilidad de la semana (p. 93) se había convertido en una tarjeta por persona con los siete días en parejas; vuelve a ser la cuadrícula de puntos, con el día encima del número. `stack={false}` marca ahora la tabla con `data-rejilla`, que en el teléfono estrecha las celdas. Lo mismo para el reloj contractual, el de Menú Diario y los festivos de Horarios (p. 102): tres columnas cortas, una fila por día, en vez de siete tarjetas. **Agente** (p. 99): centrado, con el icono del menú, la etiqueta Próximamente y "Volver a Inicio"; sigue sin hacer nada (CLAUDE.md). **Logotipo del espacio**: el campo de archivo lo pintaba el navegador en su idioma ("Choose File · No file chosen"); ahora es un botón "Elegir imagen" con el nombre elegido, como la foto del restaurante. **Iniciales**: "Marta Gil (trabajadora)" daba «M(»; se saltan las palabras que no empiezan por letra. **Lo que no se copia**: la Asignación del PDF elige un restaurante en un desplegable; la web lista todos con "Gestionar", como en escritorio. El "mm/dd/yyyy" de las fechas de Auditoría es del navegador en inglés de las capturas; en uno en español sale dd/mm/aaaa | `components/ui/Table.tsx`, `styles/tokens.css`, `equipo/TeamView.tsx`, `ajustes/page.tsx`, `ajustes/SettingsForms.tsx`, `agente/page.tsx`, `components/ui/Avatar.tsx`, `components/ui/avatar-initials.test.ts`, `components/ui/table-stack.test.tsx` |
| 24/09/2026 | **Parte 10 · Panel: Inicio y Solicitudes (páginas 110 a 123)** · barridos como la clienta de Magariños el Inicio, Actividad, la lista, Nueva, el borrador y el detalle en cinco estados. **P2 otra vez, ahora de raíz**: el detalle de cada solicitud se salía (427 px, 445 con el presupuesto) porque su rejilla era `grid lg:grid-cols-3` sin columnas por debajo de `lg`, y la columna implícita mide lo que el texto más largo sin partir. Había 153 rejillas así en 75 archivos; todas llevan ahora `grid-cols-1`, que no cambia nada en escritorio y deja encoger. `core/rejillas-con-columna-base.test.ts` recorre el código y falla si vuelve una. Toda insignia parte línea en el teléfono si no cabe, no solo las de una tabla apilada. **Seguimiento**: hasta cuatro pasos van en una fila, como la p. 122; con más (una corrección los deja en seis) iban dos por fila con la línea colgando, y en el teléfono pasan a vertical. El icono de adorno del resumen se quita en el teléfono. **Filtros**: dos por fila y no tres; en tres, cada desplegable medía 98 px y ponía "Todos lc". **Campos de archivo**: el de los adjuntos (solicitud, borrador, justificante, archivos) y el de las incidencias los pintaba el navegador en su idioma ("Choose File"); ahora son un botón "Elegir archivo", y `core/campos-de-archivo.test.ts` falla si alguno vuelve a la vista. **Lo que no se copia**: la tarjeta de contexto del panel dice "Panel de restaurante" y no el papel de cada uno ("Propietario del restaurante", "Editor"): el armazón no carga hoy el acceso de cada restaurante, y es un cambio aparte | `components/panel/RequestPieces.tsx`, `components/FileUploadField.tsx`, `components/ui/FilterBar.tsx`, `styles/tokens.css`, `ayuda/incidencias/[id]/SpaceIncidentForms.tsx`, 75 archivos con `grid-cols-1`, `core/rejillas-con-columna-base.test.ts`, `core/campos-de-archivo.test.ts`, `components/panel/request-timeline.test.tsx` |
| 24/09/2026 | **Parte 11 · Panel: Menú Diario, Mensajes y Calendario (páginas 124 a 135)** · barridos como la clienta de Magariños la lista, Crear menú, Plantillas, Servicio y consumo, un menú en sus cuatro pestañas (borrador y pendiente de asignar), Mensajes y Calendario en mes y agenda: ninguna se sale. **Lista de menús** (p. 124): "Crear menú" y "Copiar menú anterior" iban al lado de los filtros y se comían media fila; los filtros quedaban uno por línea y cortados. Van debajo, a lo ancho. La tarjeta de actualizaciones del ciclo ya no se queda en 280 px. **Aviso de cambios sin publicar** (p. 132): el texto (`flex-1`) cedía todo el sitio a los botones y salía una palabra por línea; los botones pasan a lo ancho debajo. **Vista previa** (p. 127): el menú se reduce con una escala en píxeles y a 520 px su caja lo recortaba por la derecha. `MenuPreview` acepta `mobileWidth` y pinta en el teléfono una vista de 300 px. **Plantillas** (p. 126): tres miniaturas por fila, como el PDF; a lo ancho cada una ocupaba una pantalla. Mensajes y Calendario ya seguían el diseño | `menu-diario/page.tsx`, `menu-diario/MenuSectionHeader.tsx`, `menu-diario/[menuId]/page.tsx`, `menu-diario/plantillas/page.tsx`, `components/menu/MenuPreview.tsx`, `components/menu/menu-preview-movil.test.tsx` |
| 24/09/2026 | **Parte 12 · Panel: Plan, Pagos, Informes, Archivos, Usuarios y Ajustes (páginas 136 a 157)** · barridas como la clienta de Magariños 23 rutas: Plan y el cambio o la baja, facturación con su detalle y documentos, las seis secciones de Informes y datos, Archivos, Usuarios e invitar, las cuatro pestañas de Ajustes y ayuda, el libro de consumos, las reseñas y el orden de importancia. Ninguna se sale. **Márgenes de escritorio**: quedaban dieciséis envoltorios con `p-8` fijo (la ayuda del panel, las incidencias, el libro de consumos, las reseñas, la puesta en marcha, la conversación sin permiso…) que en el teléfono sumaban 32 px por lado a los 16 del armazón. Pasan a `sm:p-8`, y `core/sin-margen-fijo-de-escritorio.test.ts` falla si vuelve uno. **Fuentes de datos**: "Como propietario puedes autorizar…" salía dos veces seguidas; la página lo pasaba como subtítulo y el bloque ya lo pone para el propietario. El subtítulo de `IntegrationsBlock` es opcional y la página no lo repite. **La barra de abajo** marcaba Inicio en Ayuda y en Fuentes: "Ajustes y ayuda" apunta a `/ajustes` y esas dos pestañas viven en `/ayuda` y `/fuentes`. `activeDestination()` las reconoce como el mismo destino. **Archivos** dos por fila (p. 149) y **el resumen de Pagos** en filas compactas (p. 138). **Lo que no se copia**: las cifras de ejemplo de Informes y datos (sesiones, mapa de calor, hallazgos): sin fuente conectada la web dice que no hay datos y por qué (CLAUDE.md) | `components/help/HelpCenter.tsx` y otros 15 con `sm:p-8`, `components/establishment/IntegrationsBlock.tsx`, `restaurantes/[id]/fuentes/page.tsx`, `components/shell/navigation.ts`, `restaurantes/[id]/archivos/page.tsx`, `restaurantes/[id]/facturacion/page.tsx`, `core/sin-margen-fijo-de-escritorio.test.ts`, `integrations-block.test.tsx`, `navigation.test.ts` |
| 24/09/2026 | **Revisión de Bosco en el teléfono: los dos Inicios** · **Inicio del espacio** (p. 22): la tarjeta de contexto pasa a ser una fila, el espacio con su rol a la izquierda y "Cambiar de espacio" a la derecha, como el dibujo; sigue siendo un solo enlace. **Inicio general** (p. 1 y G01): "Mensajes sin leer" es una fila —icono, título con su frase, botón— sin cabecera de tarjeta; antes la frase se partía en cuatro líneas. Sigue sin ir el selector de mes del saludo (solo hay mes en curso) | `components/shell/AppShell.tsx`, `(global)/page.tsx` |
| 24/09/2026 | **Cerrado el punto 2**: las doce partes del diseño móvil (157 páginas) comparadas y ajustadas en la web. Quedan como diferencias escritas, no como pendientes: los tamaños de letra del PDF, los datos de ejemplo, los formularios largos a una columna, los filtros a dos por fila y el papel de cada persona en la tarjeta de contexto del panel | — |
